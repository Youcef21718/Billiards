...
def plot_arrow(a,v):
    plt.arrow(a[0],a[1],v[0],v[1], fc='r', ec='r', head_width=0.05, head_length=0.1, overhang=0.1, linewidth = 1)
...
def count_collisions(index,real_size):
    collisions = 0
    size = 2*real_size
...
while(iterations>0):
    ...
    if(iterations%100 == 0): print("Total energy: ",get_energy(index))
    if(iterations%100 == 0): print("Total momentum: ",get_momentum(index))

    collisions_list = np.zeros((hexEdges(size),2))
    collisions_list = collisions_list.astype(int)

    collisions = count_collisions(index,real_size)
    x.append(collisions)

    if(collisions==0):
        break
    if(iterations%100 == 0): print(collisions,"collisions")
    my_collision = rand(collisions)

    index0 = collisions_list[my_collision][0]
    index1 = collisions_list[my_collision][1]
    if(iterations%100 == 0): print("Ball",index0,"collided with ball",index1)
    if(iterations%100 == 0): print("*******************************************")
